import defaultTheme, { Theme } from '@hola-ui/theme'
import { isFunction, mergeWith } from '@hola-ui/utils'
import { ColorHues } from '@hola-ui/theme/dist/types/foundations/colors'
...
/**
 * Function to override or customize the Hola UI theme conveniently
...
export function extendTheme<T extends ThemeOverride>(
  ...
  function customizes(source: unknown, override: unknown) {
    if (isFunction(source)) {
      return (...args: unknown[]) => {
        ...
        return mergeWith({}, sourceValue, overrideValue, customizes)
      }
    }
  ...
  return mergeWith({}, baseTheme, overrides, customizes)
}
